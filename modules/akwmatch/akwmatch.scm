...
;;; (define match:gentemp gensym)
(define-macro (match:gentemp) '(gensym))

(define match:error
  (lambda (val . args)
    #; (for-each (lambda (expr) (format (current-error-port) "~s" expr)) args)
    (##raise-expression-parsing-exception 'ill-formed-special-form  'match "no matching clause for ~m" val args)))
...
(define match:syntax-err
  (lambda (obj msg)
    ;; Ypsilon: (syntax-violation 'match (format "~a ~s" msg obj))
    #;(##raise-expression-parsing-exception 'ill-formed-special-form  'match msg obj)
    (error msg obj)))
    ...
    (let* ((length>= (match:gentemp))
           ...
           (plist (map (lambda (c)
                         (let* ((x (bound (validate-pattern (car c))))
                           ...
                           (list p code bv (and fail (match:gentemp)) #f)))
                       clauses))
           (code (gen x '() plist (cdr eb-errf) length>= (match:gentemp))))
...
(define genletrec
  (lambda (pat exp body match-expr)
    (let* ((length>= (match:gentemp))
           ...
           (x (match:gentemp))
           (m (gen x '() plist (cdr eb-errf) length>= (match:gentemp)))
           (gs (map (lambda (_) (match:gentemp)) bv)))
      (unreachable plist match-expr)
  ...
  (lambda (match-expr)
    (cond ((eq? match:error-control 'unspecified)
          ...
          ((eq? match:error-control 'match)
           (let ((errf (match:gentemp)) (arg (match:gentemp)))
                      ...
                      (lambda (q b)
                        (let ((bvars (find-prefix b a)))
                          (k `(,q ,(cadr p)
                                  ...
                                  ,(match:gentemp)
                                  ,(match:gentemp)
                                  ,(map (lambda (_) (match:gentemp)) bvars))
                             b)))))
              ((and (pair? p) (eq? '$ (car p)))
...
;;; end of expanders
  (module-match-end)) ;; Closing the `let` at end of the header.
(eval '(define-macro (match:gentemp) '(gensym)))
(eval '(define-macro (match . args)
         (cond ((and (list? args)
                     (<= 1 (length args))
                     (match:andmap
                ...
                (let* ((exp (car args))
                       (clauses (cdr args))
                       (e (if (symbol? exp) exp (match:gentemp))))
                  (if (symbol? exp)
                      (genmatch e clauses `(match ,@args))
                      `(let ((,e ,exp))
                         ,(genmatch e clauses `(match ,@args))))))
               (else (match:syntax-err `(match ,@args) "syntax error in")))))
(eval '(define-macro (match-lambda . args)
         (if (and (list? args)
                  (match:andmap
                   (lambda (arg)
                     (and (pair? arg) (list? (cdr arg)) (pair? (cdr arg))))
                   args))
             (let ((e (match:gentemp))) `(lambda (,e) (match ,e ,@args)))
...
(eval '(define-macro (match-lambda* . args)
         (if (and (list? args)
             ...
             (let ((e (match:gentemp))) `(lambda ,e (match ,e ,@args)))
             (match:syntax-err `(match-lambda* ,@args) "syntax error in"))))
(eval '(define-macro (match-let . args)
         (let ((g158 (lambda (pat exp body) `(match ,exp (,pat ,@body))))
               (g154 (lambda (pat exp body)
                       (let ((g (map (lambda (x) (match:gentemp)) pat))
               ...
               (g145 (lambda (p1 e1 p2 e2 body)
                       (let ((g1 (match:gentemp)) (g2 (match:gentemp)))
                         `(let ((,g1 ,e1) (,g2 ,e2))
                            (match (cons ,g1 ,g2)
                                   ((,p1 unquote p2) ,@body))))))
           ...
           (if (pair? args)
               (if (null? (car args))
                   ...
                   (if (and (pair? (car args))
                       ...
                       (g176)))
               (g176)))))
(eval '(define-macro (match-letrec . args)
           ...
           (if (pair? args)
               (if (list? (car args))
                   ...
                   (if (pair? (car args))
                       ...
                       (g195)))
               (g195)))))
(eval '(define-macro (match-define . args)
           ...
           (if (pair? args)
               (if (g210 (car args))
                   ...
                   (if (and (pair? (cdr args)) (null? (cddr args)))
                       ...
                       (g209)))
               (g209)))))
(eval '(define-macro (define-values names . body)
         (let ((arg1 (gensym 'arg1)) (rest (gensym 'rest)))
           `(begin
              ,@(map (lambda (name) `(define ,name #f)) (cdr names))
              (define ,(car names)
                (call-with-values
                 (lambda () unquote body)
                 (lambda (,arg1 unquote rest)
                   (begin
                     ,@(map (lambda (name)
                              `(set! ,name
                                     (let ((,name (car ,rest)))
