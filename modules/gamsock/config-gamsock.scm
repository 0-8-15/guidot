...
(define platform-gcc '"gcc")

#;(define platform-gcc
  "gcc -isysroot /usr/i686-w64-mingw32 -DWIN32 -DWINVER=0x0501")

#;(define platform-gcc
  "gcc -I- -I /usr/i686-w64-mingw32/include  -DWIN32 -DWINVER=0x0501")

(define cross-gcc
  "/usr/bin/i686-w64-mingw32-gcc -isysroot /usr/i686-w64-mingw32 -DWIN32 -DWINVER=0x0501")


(define cross-gcc platform-gcc)
...
(define (try-compiling c-doc)
  (let* ((fn (generate-temp-file-name "gamsoc"))
	...
	(lambda ()
	  ...
	  (let* ((result
		  (shell-command
		   (string-append "/bin/sh -c \"" cross-gcc " -c "
				  fnc
				  " -o "
				  fno
				  ">/dev/null 2>/dev/null\""))))
	...
	(lambda ()
	  ...
	  (let* ((result
		  (begin
                    ...
                    (shell-command
                     (string-append "/bin/sh -c \"" platform-gcc " "
                                    fno
                                    " -o "
                                    fn
                                    #;">/dev/null 2>/dev/null"
	    ...
	    (if (zero? result) fn #f)))
	(lambda ()
	  #t #;(if (file-exists? fnc) (delete-file fnc))))))
